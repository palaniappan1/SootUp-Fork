...
public abstract class BaseFixJarsTest {
    ...
    public JavaView supplyJavaView(String jarName) {
        AnalysisInputLocation inputLocation = new JavaClassPathAnalysisInputLocation(getJarPath(jarName));
        return new JavaView(inputLocation);
    }

    public void assertMethodConversion(String methodSignature, String jarName){
        try {
            supplyJavaView(jarName).getIdentifierFactory().parseMethodSignature(methodSignature);
        }
        catch (Exception e) {
            e.printStackTrace();
        }
